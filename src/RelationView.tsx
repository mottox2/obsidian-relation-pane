      ...
      <div className="backlink-pane">
        <div className="tree-item-self">
          ...
          <div className="tree-item-flair-counter">
            <div className="tree-item-flair">{links.length}</div>
          </div>
        </div>
        <div className="search-result-container">
          {links.map((link) => {
            return (
              ...
              >
                <LinkIcon /> {extractExt(link)}
              </div>
            )
          })}
          {links.length === 0 && (
            <div className="search-empty-state">No links found.</div>
          )}
        </div>
        <div className="tree-item-self">
          <div className="tree-item-inner">Backlinks</div>
          <div className="tree-item-flair-counter">
            <div className="tree-item-flair">{backLinks.length}</div>
          </div>
        </div>
        <div className="search-result-container">
          {backLinks.map((link) => {
          ...
          {backLinks.length === 0 && (
            <div className="search-empty-state">No backlinks found.</div>
          )}
        </div>
        ...
        <div className="tree-item-self">
          ...
          <div className="tree-item-flair-counter">
            <div className="tree-item-flair">{newLinks.length}</div>
          </div>
        </div>
        <div className="search-result-container">
          {newLinks.map((link) => {
            return (
              ...
              >
                <NewLinkIcon /> {extractExt(link)}
              </div>
            )
          })}
          {newLinks.length === 0 && (
            <div className="search-empty-state">No new links found.</div>
          )}
        </div>
        {false && (
          <div>
            ...
            <p>resolvedLinks</p>
            <pre>
              {JSON.stringify(this.app.metadataCache.resolvedLinks, null, 2)}
            </pre>
            <p>unresolvedLinks</p>
            <pre>
              {JSON.stringify(this.app.metadataCache.unresolvedLinks, null, 2)}
            </pre>
          </div>
        )}
      </div>
